...
<div class="px-8 py-8" x-data="partnerManager()">
    <div class="mb-8 flex justify-between items-end">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Mitra / Klien</h2>
            <p class="text-slate-500 mt-1">Kelola logo, nama, dan deskripsi mitra bisnis Anda.</p>
        </div>
    </div>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <select name="display_type" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none" required>
                                    <option value="logo">Logo Saja (Default)</option>
                            ...
                            <div>
                                ...
                                <textarea name="description" rows="3" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all" placeholder="Deskripsi mengenai mitra..."></textarea>
                            </div>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col gap-2">
                                        <span class="inline-block px-2 py-1 bg-slate-100 text-slate-600 text-[10px] font-bold rounded-md uppercase tracking-wider max-w-fit">
                                            <?= htmlspecialchars($partner['display_type'] ?? 'logo') ?>
                                        </span>
                                        <?php if (!empty($partner['logo_path']) && file_exists(BASE_PATH . '/public' . $partner['logo_path'])): ?>
                                            <img src="<?= BASE_URL . $partner['logo_path'] ?>" class="h-10 w-auto object-contain bg-white rounded border border-slate-200 p-1">
                                        <?php else: ?>
                                            <span class="text-xs font-semibold text-slate-400 bg-slate-100 px-2 py-1 rounded-md">Tanpa Logo</span>
                                        <?php endif; ?>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    ...
                                    <?php if (!empty($partner['description'])): ?>
                                        <p class="text-xs text-slate-500 mt-1 line-clamp-2"><?= htmlspecialchars($partner['description']) ?></p>
                                    <?php endif; ?>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <button @click='openEditModal(<?= json_encode($partner) ?>)' class="w-8 h-8 rounded-lg bg-blue-50 text-blue-500 hover:bg-blue-500 hover:text-white transition-colors inline-flex items-center justify-center mr-1">
    ...
    <div x-show="editModal" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div x-show="editModal"
            ...
            <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <h3 class="text-xl font-bold text-slate-800">Edit Mitra</h3>
            ...
            <div class="p-6 overflow-y-auto">
                <form :action="editFormAction" method="POST" enctype="multipart/form-data" id="editForm">
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">Logo Perusahaan (Ganti Baru)</label>
                            <input type="file" name="logo" accept="image/*" class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">Deskripsi Singkat (Opsional)</label>
                            <textarea name="description" x-model="editData.description" rows="3" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all"></textarea>
                        </div>
                    </div>
                </form>
            </div>
    ...
    Alpine.data('partnerManager', () => ({
        ...
        openEditModal(item) {
            this.editData = {
                ...
                display_type: item.display_type || 'logo'
            };
            this.editFormAction = '<?= BASE_URL ?>/settings/partners/update/' + item.id;
            this.editModal = true;
        }
    }));
});
